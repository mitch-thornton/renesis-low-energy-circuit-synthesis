	  ...
	  m_verbose(verbose)
{
	for (auto &pairs : m_pairs) {
		/* ADSHIM PATCH (v61): upstream reserved n_cubes^2 entries, which
		 * for minterm-seeded covers (2^k cubes) allocates gigabytes up
		 * front.  Capacity does not affect results; cap the hint. */
		const std::size_t hint = original.size() * original.size();
		pairs.reserve(hint < (1u << 20) ? hint : (1u << 20));
	}
